...
# -----------------------------
# Login por código enviado por email (código válido por 10 minutos)
# -----------------------------
def _coluna_email_planilha(df_usuarios):
    """Retorna o nome da coluna de e-mail na planilha (EMAIL, E-mail, email, etc.)."""
    for col in df_usuarios.columns:
        if str(col).strip().upper() == 'EMAIL' or str(col).strip().lower() == 'e-mail':
            return col
    return None

def buscar_usuario_por_email(email):
    """Retorna o dicionário do usuário se o e-mail estiver cadastrado na planilha login_senha."""
    df_usuarios = carregar_usuarios()
    if df_usuarios is None:
        return None
    col_email = _coluna_email_planilha(df_usuarios)
    if not col_email:
        return None
    ...
    for idx, row in df_usuarios.iterrows():
        ...
        if str(val).strip().lower() == email_limpo:
            ...
            cpf_usuario = re.sub(r'[^0-9]', '', str(cpf_val)) if not pd.isna(cpf_val) else ''
            if inep_val != '' and not pd.isna(inep_val):
                inep_usuario = re.sub(r'[^0-9]', '', str(int(float(inep_val))))
            else:
                inep_usuario = ''
            return {
...
def _gerar_codigo_6():
    """Gera código de 6 dígitos para login."""
    return ''.join(random.choices(string.digits, k=6))

def _get_gmail_credentials():
    """Obtém GMAIL_USER e GMAIL_PASSWORD de variáveis de ambiente ou Streamlit secrets."""
    user = os.getenv('GMAIL_USER', '').strip()
    senha = os.getenv('GMAIL_PASSWORD', '').strip()
    if hasattr(st, 'secrets'):
        if not user and 'GMAIL_USER' in st.secrets:
            user = str(st.secrets.get('GMAIL_USER', '')).strip()
        if not senha and 'GMAIL_PASSWORD' in st.secrets:
            senha = str(st.secrets.get('GMAIL_PASSWORD', '')).strip()
    return user, senha

def enviar_codigo_login(email):
    """Envia código de 6 dígitos por email. Código válido por 10 minutos. Retorna (sucesso, mensagem)."""
    usuario = buscar_usuario_por_email(email)
    if not usuario:
        return False, "E-mail não cadastrado na planilha de acesso. Entre em contato com o administrador."
    gmail_user, gmail_password = _get_gmail_credentials()
    if not gmail_user or not gmail_password:
        return False, "Envio de e-mail não configurado (GMAIL_USER/GMAIL_PASSWORD). Configure em Secrets do app."
    codigo = _gerar_codigo_6()
    expira_em = datetime.now() + timedelta(minutes=10)
    if 'login_codigo' not in st.session_state:
        st.session_state.login_codigo = {}
    st.session_state.login_codigo = {
...
"""
    try:
        ...
        server.quit()
        return True, f"Código enviado para {email}. Verifique sua caixa de entrada (e spam). Válido por 10 minutos."
    except Exception as e:
        return False, f"Erro ao enviar e-mail: {str(e)}"

def validar_codigo_login(email, codigo_digitado):
    """Valida o código informado. Retorna dict do usuário se válido, None caso contrário."""
    data = st.session_state.get('login_codigo')
    if not data:
        return None
    email_limpo = str(email).strip().lower()
    if data['email'] != email_limpo:
        return None
    if datetime.now() > data['expira_em']:
        return None  # expirado
    if str(codigo_digitado).strip() != str(data['codigo']):
        return None
    usuario = data.get('usuario')
    if not usuario:
        return None
    # Limpar código após uso
    if 'login_codigo' in st.session_state:
        del st.session_state.login_codigo
    # Registrar acesso no Firebase se disponível
    if MONITORING_AVAILABLE and usuario:
        try:
            client_info = get_client_info()
            firebase_manager.log_access(
                usuario=usuario.get('nome', 'Usuário'),
...
def tela_instrucoes():
    ...
    **1.1 - Acesse o SGE:**
    ...
    - Faça login com seu e-mail cadastrado (código enviado por e-mail, válido 10 min)
    ...
    **1.4 - Baixe a Planilha:**
    ...
    st.markdown("""
    **2.1 - Faça Login:**
    - Digite seu e-mail cadastrado na planilha de acesso (login_senha)
    - Clique em "Enviar código por e-mail" e insira o código recebido (válido 10 minutos)
    - Clique em "Entrar"
...
def tela_login():
    """Exibe tela de login: acesso apenas por e-mail, com código enviado por e-mail (válido 10 min)."""
    ...
    with col2:
        ...
        st.info("Acesso apenas por e-mail. O código será enviado para o e-mail cadastrado na planilha de acesso (login_senha). Válido por 10 minutos.")
        
        # Avisar se a planilha não tiver coluna de e-mail
        df_usuarios = carregar_usuarios()
        if df_usuarios is not None and _coluna_email_planilha(df_usuarios) is None:
            st.warning("A planilha **login_senha.xlsx** precisa ter uma coluna **EMAIL** (ou **E-mail**) com os e-mails autorizados.")
        
        # Se já enviou código nesta sessão, mostrar campo do código
        data_codigo = st.session_state.get('login_codigo')
        aguardando_codigo = data_codigo and (datetime.now() <= data_codigo.get('expira_em', datetime.min))
        
        if aguardando_codigo:
            email_para_codigo = data_codigo.get('email', '')
            with st.form("login_codigo_form"):
                st.text_input("E-mail", value=email_para_codigo, disabled=True, key="email_readonly")
                codigo = st.text_input("Código recebido por e-mail:", placeholder="Digite o código de 6 dígitos", max_chars=6, help="Código válido por 10 minutos")
                col_btn1, col_btn2 = st.columns(2)
                with col_btn1:
                    entrar_btn = st.form_submit_button("Entrar", use_container_width=True)
                with col_btn2:
                    outro_email_btn = st.form_submit_button("Usar outro e-mail", use_container_width=True)
            ...
            if outro_email_btn:
                ...
                st.rerun()
            if entrar_btn:
                if not codigo or len(codigo.strip()) != 6:
                    st.error("Digite o código de 6 dígitos enviado para seu e-mail.")
                else:
                    usuario = validar_codigo_login(email_para_codigo, codigo.strip())
                    if usuario:
            ...
            with st.form("login_email_form"):
                email = st.text_input("E-mail:", placeholder="Digite seu e-mail cadastrado", type="default", help="Use o e-mail cadastrado na planilha login_senha")
                enviar_btn = st.form_submit_button("Enviar código por e-mail", use_container_width=True)
            
            if enviar_btn:
                if not email or not str(email).strip():
                    st.error("Digite seu e-mail.")
                else:
                    sucesso, msg = enviar_codigo_login(str(email).strip())
                    if sucesso:
                        st.success(msg)
                        st.rerun()
                    else:
...
def processar_notas_frequencia(df):
    ...
    # Planilha Ata/Mapa: normaliza Periodo; o recorte ao 1º bimestre (notas) é feito na interface principal
...
def filtrar_apenas_bimestre_1(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return filtrar_bimestre_n(df, 1)


def filtrar_bimestre_n(df: pd.DataFrame, n: int) -> pd.DataFrame:
    """Mantém somente linhas do bimestre n (coluna Periodo)."""
    if "Periodo" not in df.columns:
        return df.iloc[0:0].copy()
    mask = df["Periodo"].apply(lambda p: mapear_bimestre(p) == n)
    return df[mask].copy()


def tem_dados_bimestre(df: pd.DataFrame, n: int) -> bool:
    if df is None or df.empty or "Periodo" not in df.columns:
        return False
    return bool(df["Periodo"].apply(lambda p: mapear_bimestre(p) == n).any())


def montar_freq_detalhada_por_aluno(
    df_src: pd.DataFrame,
    coluna_aluno: str,
    modo: str,
    classificar_fn,
    ...
    """
    Tabela por aluno+turma para abas de frequência.
    modo: 'anual' | 'bimestre_1' | 'bimestre_2'
    """
    if df_src is None or df_src.empty:
        return pd.DataFrame()

    if modo == "anual":
        if "Frequencia Anual" not in df_src.columns:
            return pd.DataFrame()
        agg_map = {"Frequencia": ("Frequencia Anual", consolidar_fn)}
        grp_cols = [coluna_aluno, "Turma"] if "Turma" in df_src.columns else [coluna_aluno]
        freq_det = df_src.groupby(grp_cols, as_index=False).agg(**agg_map)
    else:
        if "Frequencia" not in df_src.columns:
            return pd.DataFrame()
        num_bim = 1 if modo == "bimestre_1" else 2
        df_calc = filtrar_bimestre_n(df_src, num_bim)
        if df_calc.empty:
            return pd.DataFrame()
        col_faltas = f"Faltas_{num_bim}_Bimestre"
        agg_map = {"Frequencia": ("Frequencia", _media_frequencia_disciplinas)}
        if "Falta" in df_calc.columns:
            agg_map[col_faltas] = ("Falta", "sum")
        grp_cols = [coluna_aluno, "Turma"] if "Turma" in df_calc.columns else [coluna_aluno]
        freq_det = df_calc.groupby(grp_cols, as_index=False).agg(**agg_map)

    if "Falta" in freq_det.columns:
        freq_det["Falta"] = pd.to_numeric(freq_det["Falta"], errors="coerce").fillna(0).astype(int)
    freq_det["Classificacao_Freq"] = freq_det["Frequencia"].apply(classificar_fn)
    freq_det = freq_det.sort_values("Frequencia", ascending=True, na_position="last")
    return freq_det
...
# Barra de navegação com opções do usuário (acesso é só por e-mail + código; sem alterar senha)
...
try:
    ...
    if tipo_planilha == 'conteudo_aplicado':
        ...
        # Guardar todos os períodos para abas de frequência (anual / 1º / 2º bimestre)
        if "Periodo" in df.columns:
            st.session_state["df_freq_todos_periodos"] = df.copy()
        else:
            st.session_state["df_freq_todos_periodos"] = None

        # Notas e indicadores: apenas 1º bimestre
        if "Periodo" in df.columns:
...
# Mesmos filtros da barra lateral, com todos os bimestres (frequência detalhada por aba)
_df_freq_base = st.session_state.get("df_freq_todos_periodos")
if _df_freq_base is not None:
    df_filt_freq = _df_freq_base.copy()
    if escola_sel != "Todas":
        df_filt_freq = df_filt_freq[df_filt_freq["Escola"] == escola_sel]
    if status_sel:
        df_filt_freq = df_filt_freq[df_filt_freq["Status"].isin(status_sel)]
    if turma_sel:
        df_filt_freq = df_filt_freq[df_filt_freq["Turma"].isin(turma_sel)]
    if disc_sel:
        df_filt_freq = df_filt_freq[df_filt_freq["Disciplina"].isin(disc_sel)]
    if aluno_sel != "Todos":
        df_filt_freq = df_filt_freq[df_filt_freq[coluna_aluno] == aluno_sel]
else:
    df_filt_freq = df_filt.copy()
...
if "Frequencia Anual" in df_filt.columns or "Frequencia" in df_filt.columns:
    ...
    def _render_cards_resumo_freq(contagem_freq_geral):
        total_alunos_freq = contagem_freq_geral.sum()
        colF1, colF2, colF3, colF4, colF5 = st.columns(5)
        cards = [
            (colF1, "< 75% (Reprovado)", "Reprovado", "#dbeafe", "#bfdbfe", "#3b82f6", "#1e40af"),
            (colF2, "< 80% (Alto Risco)", "Alto Risco", "#e0f2fe", "#b3e5fc", "#0ea5e9", "#0c4a6e"),
            (colF3, "< 90% (Risco Moderado)", "Risco Moderado", "#f0f9ff", "#dbeafe", "#1e40af", "#1e40af"),
            (colF4, "< 95% (Ponto Atenção)", "Ponto de Atenção", "#eff6ff", "#dbeafe", "#3b82f6", "#1e40af"),
            (colF5, "≥ 95% (Meta Favorável)", "Meta Favorável", "#dbeafe", "#bfdbfe", "#3b82f6", "#1e40af"),
        ]
        for col_ui, rotulo, chave, c1, c2, borda, texto in cards:
            valor = contagem_freq_geral.get(chave, 0)
            percent = (valor / total_alunos_freq * 100) if total_alunos_freq > 0 else 0
            with col_ui:
                st.markdown(f"""
                <div style="background: linear-gradient(135deg, {c1}, {c2}); border-radius: 10px; padding: 15px; margin: 5px 0; box-shadow: 0 2px 8px rgba(59, 130, 246, 0.15); border-left: 4px solid {borda};">
    ...
    if "Frequencia Anual" in df_filt.columns:
        blocos_freq.append((
            ...
            "Frequencia Anual",
            "Coluna <strong>Frequência Anual</strong> da planilha — resultado acumulado do ano letivo.",
        ))
    if "Frequencia" in df_filt.columns and df_filt["Frequencia"].notna().any():
        blocos_freq.append((
            "1º Bimestre",
            "Frequencia",
            "Média da coluna <strong>Frequência</strong> por aluno em todas as disciplinas do Primeiro Bimestre.",
        ))
    elif "Frequencia" in df_filt.columns and not blocos_freq:
        blocos_freq.append((
            "Frequência",
            "Frequencia",
            "Média da coluna <strong>Frequência</strong> por aluno em todas as disciplinas do período.",
        ))

    for titulo_bloco, col_freq_resumo, descricao_bloco in blocos_freq:
        st.markdown(f"""
        <div style="margin: 16px 0 8px 0;">
            <h4 style="color: #1e40af; margin: 0 0 6px 0; font-size: 1.15em; font-weight: 700;">{titulo_bloco}</h4>
            <p style="color: #64748b; margin: 0; font-size: 0.9em;">{descricao_bloco}</p>
        </div>
        """, unsafe_allow_html=True)
...
def _consolidar_valor_frequencia_series(s):
    """
    Para o mesmo aluno+turma a planilha repete a frequência em várias disciplinas; às vezes com
    arredondamentos levemente diferentes. Usa o valor mais repetido (moda); se não houver moda, o primeiro válido.
    """
...
# Seção expandível: Análise Detalhada de Frequência (abas Anual / 1º / 2º bimestre)
with st.expander("Análise Detalhada de Frequência"):
    if "Frequencia Anual" in df_filt_freq.columns or "Frequencia" in df_filt_freq.columns:
        ...
        abas_freq_cfg = []
        if "Frequencia Anual" in df_filt_freq.columns:
            abas_freq_cfg.append((
                "Anual",
                "anual",
                "Frequência — Anual",
                "Coluna <strong>Frequência Anual</strong> da planilha. Ordenado da menor para a maior %.",
            ))
        if "Frequencia" in df_filt_freq.columns:
            abas_freq_cfg.append((
                "1º Bimestre",
                "bimestre_1",
                "Frequência — 1º Bimestre",
                "Média da coluna <strong>Frequência</strong> por aluno em todas as disciplinas do 1º Bimestre. "
                "Ordenado da menor para a maior %.",
            ))
        if tem_dados_bimestre(df_filt_freq, 2) and "Frequencia" in df_filt_freq.columns:
            abas_freq_cfg.append((
                "2º Bimestre",
                "bimestre_2",
                "Frequência — 2º Bimestre",
                "Média da coluna <strong>Frequência</strong> por aluno em todas as disciplinas do 2º Bimestre. "
                "Ordenado da menor para a maior %.",
            ))

        tabs_freq = st.tabs([cfg[0] for cfg in abas_freq_cfg])

        for tab_ui, (rotulo, modo, titulo_aba, descricao_aba) in zip(tabs_freq, abas_freq_cfg):
            with tab_ui:
                freq_detalhada = montar_freq_detalhada_por_aluno(
                    df_filt_freq,
                    coluna_aluno,
                    modo,
                    classificar_frequencia,
                    _consolidar_valor_frequencia_series,
                )
                if freq_detalhada.empty:
                    st.info(f"Sem dados de frequência para **{rotulo}** com os filtros atuais.")
                    continue

                st.markdown(f"#### {titulo_aba}")
                st.markdown(
                    f'<p style="color:#64748b;font-size:0.9em;margin:0 0 12px 0;">{descricao_aba}</p>',
                    unsafe_allow_html=True,
                )
                ...
                cols_freq_view = [coluna_aluno]
                if "Turma" in freq_detalhada.columns:
                    cols_freq_view.append("Turma")
                for col_extra in ("Falta", "Faltas_1_Bimestre", "Faltas_2_Bimestre"):
                    if col_extra in freq_detalhada.columns:
                        cols_freq_view.append(col_extra)
                ...
                col_export5, _ = st.columns([1, 4])
                with col_export5:
                    slug = rotulo.lower().replace("º", "o").replace(" ", "_")
                    if st.button(
                        f"📊 Exportar {rotulo}",
                        key=f"export_frequencia_{slug}",
                        help=f"Baixar planilha — frequência {rotulo}",
                    ):
                        excel_data = criar_excel_formatado(
                            freq_detalhada[cols_freq_view], f"Freq_{slug}"
                        )
                        st.download_button(
                            label="Baixar Excel",
                            data=excel_data,
                            file_name=f"analise_frequencia_{slug}.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            key=f"dl_freq_{slug}",
                        )
            ...
            if "Frequencia Anual" in df_filt.columns or "Frequencia" in df_filt.columns:
                if "Frequencia Anual" in df_filt.columns:
                    freq_detalhada = df_filt.groupby([coluna_aluno, "Turma"], as_index=False).agg(
                        Frequencia=("Frequencia Anual", _consolidar_valor_frequencia_series),
                        **({"Falta": ("Falta", "sum")} if "Falta" in df_filt.columns else {}),
                    )
                else:
                    freq_detalhada = df_filt.groupby([coluna_aluno, "Turma"], as_index=False).agg(
                        Frequencia=("Frequencia", _consolidar_valor_frequencia_series),
                        **({"Falta": ("Falta", "sum")} if "Falta" in df_filt.columns else {}),
                    )
